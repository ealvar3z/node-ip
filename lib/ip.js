...
function _extractIPv4FromMapped(addr) {
  ...
  if (buff[10] !== 0xff || buff[11] !== 0xff) {
    return null;
  }

  return `${buff[12]}.${buff[13]}.${buff[14]}.${buff[15]}`;
}
...
function _isIPv6Loopback(addr) {
  ...
  if (allZero && (buff[15] === 0 || buff[15] === 1)) {
    return true;
  }
  return false;
}
...
ip.isPrivate = function (addr) {
  ...
  return /^10\.([0-9]{1,3})\.([0-9]{1,3})\.([0-9]{1,3})$/
    .test(normalizedAddr)
